...
@section('content')
    <h1 class="mt-3 mb-5">Our Team</h1>
    @foreach ($chefs as $chef)
        <div class="row my-4">
            <div class="col-12 col-md-4">
                <img src="{{ asset('images/chefs/' . $chef->image) }}" alt="{{ $chef->name }}" class="img-fluid rounded-circle">
            </div>
            <div class="col-12 col-md-8">
                <h3>{{ $chef->name }}</h3>
                <p>{{ $chef->bio }}</p>
                <div class="d-flex">
                    @if($chef->social_links['facebook'])
                    <a href="{{ $chef->social_links['facebook'] }}" class="btn btn-outline-dark rounded-circle me-2">
                    ...
                    @endif
                    @if($chef->social_links['instagram'])
                    <a href="{{ $chef->social_links['instagram'] }}" class="btn btn-outline-dark rounded-circle">
